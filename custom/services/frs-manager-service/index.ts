...
export class FRSManagerServer {
    ...
    static initializer: ((this: FRSManagerServer) => void)[] = [];

    /// use for fixed config frs
    private static _sharedInstance: FRSManagerServer;
    static async sharedInstance(): Promise<FRSManagerServer> {
    ...
    initial(config: IServerConfig) {
        this.config = config;
        /// initialize
        FRSManagerServer.initializer.forEach( (init) => init.call(this) );
